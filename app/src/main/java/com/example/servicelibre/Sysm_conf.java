...
@SuppressLint("NewApi")
public class Sysm_conf extends Activity {
	Intent intent;
	BizApp bizApp;
	BizMenu bizMenu;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			_address = bizApp.GetParam(Parametres.ParaKey.ADDRESS);
			_database = bizApp.GetParam(Parametres.ParaKey.DATABASE);
			_usename = bizApp.GetParam(Parametres.ParaKey.MASTER);
			_password = bizApp.GetParam(Parametres.ParaKey.DB_PASSWORD);
			network = bizApp.GetFBDB(_address, _database, _usename, _password);
		} catch (Exception e) {
		...
		if (network) {
			...
			try {
				edt1.setText(_address);
				edt2.setText(_database);
				edt3.setText(_usename);
				edt4.setText(_password);
				...
				List<Formule> __listFormule = bizMenu.SelectFormule();
				List<Formule> __listFormuleCache = bizMenu
						.SelectAvailableFormule();
				__listFormuleCache.clear();
				for (Formule formul : __listFormule) {
					if (listFormuleAva.contains(formul.Id)) {
						bizMenu.setFormule(formul);
					}
				}
				__listFormuleCache = bizMenu.SelectAvailableFormule();

				bizMenu.SaveAvailableFormules(__listFormuleCache);
	...
	public void onClickOkButton(View view) {
		...
		String _totaltime = edt5.getText().toString();
		String _turntime = edt6.getText().toString();
		String _orderlimit = edt7.getText().toString();
		String _turn = edt8.getText().toString();

		try {
			Integer.parseInt(_totaltime);
			Integer.parseInt(_turntime);
			Integer.parseInt(_orderlimit);
			Integer.parseInt(_turn);
		...
		int t1 = Integer.parseInt(_totaltime) * 60;
		int t2 = Integer.parseInt(_turntime) * 60;
		try {
			...
			bizApp.SetParam(Parametres.ParaKey.COMMANDE_PAR_PERSON, _orderlimit);
			bizApp.SetParam(Parametres.ParaKey.TURN, _turn);
		...
		intent.setClass(Sysm_conf.this, Config.class);
	...
	public void onClickTestButton(View view) {
		...
		EditText edt3 = (EditText) findViewById(R.id.sysconfig_EditText3);
		EditText edt4 = (EditText) findViewById(R.id.sysconfig_EditText4);
		String _adress = edt1.getText().toString();
		String _database = edt2.getText().toString();
		String _usename = edt3.getText().toString();
		String _password = edt4.getText().toString();

		boolean isConnect = false;
		try {
			isConnect = bizApp.GetFBDB(_adress, _database, _usename, _password);
		} catch (Exception e) {
			AlertDialog.Builder builder = new AlertDialog.Builder(this);
		...
		if (!isConnect) {
			...
			try {
				...
				bizApp.SetParam(Parametres.ParaKey.ADDRESS, _adress);
				bizApp.SetParam(Parametres.ParaKey.DATABASE, _database);
				bizApp.SetParam(Parametres.ParaKey.MASTER, _usename);
				bizApp.SetParam(Parametres.ParaKey.DB_PASSWORD, _password);

				String _orderlimit = bizApp
						.GetParam(Parametres.ParaKey.COMMANDE_PAR_PERSON);
				String _turn = bizApp.GetParam(Parametres.ParaKey.TURN);
				EditText edt5 = (EditText) findViewById(R.id.sysconfig_editText5);
				EditText edt6 = (EditText) findViewById(R.id.sysconfig_editText6);
				EditText edt7 = (EditText) findViewById(R.id.sysconfig_editText7);
				EditText edt17 = (EditText) findViewById(R.id.sysconfig_editText17);
	...
	public void onClickModifyFormuleButton(View view) {
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		ScrollView sc = new ScrollView(Sysm_conf.this);
		LinearLayout layout = new LinearLayout(Sysm_conf.this);
		sc.addView(layout);
	...
	class LongPressedPrinterListener implements OnLongClickListener {
		...
		public boolean onLongClick(View arg0) {
			...
			defaultPrinter = new CheckBox(Sysm_conf.this);
			String _defa = new String();
			try {
				_defa = bizApp.GetParam(Parametres.ParaKey.PRINTER_DF_ID);
			} catch (Exception e) {
			...
			if (_defa.contentEquals(printer.Id)) {
			...
			for (Script script : listScript) {
				CheckBox box = new CheckBox(Sysm_conf.this);
	...
	public void onClickModifyPasswordButton(View view) {
		final View layout = LayoutInflater.from(Sysm_conf.this).inflate(
		...
		builder.setPositiveButton(res.getText(R.string.OK),
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int whichButton) {
						// TODO Auto-generated method stub
						String _s1 = ((EditText) layout
								.findViewById(R.id.modify_password_editText1))
								.getText().toString();
						String _s2 = ((EditText) layout
								.findViewById(R.id.modify_password_editText2))
								.getText().toString();
						String _s3 = ((EditText) layout
								.findViewById(R.id.modify_password_editText3))
								.getText().toString();
						boolean isgoodpass = false;
						try {
							isgoodpass = bizApp.AdminLogin(_s1);
							Integer.parseInt(_s1);
							Integer.parseInt(_s2);
							Integer.parseInt(_s3);
						} catch (Exception e) {
							AlertDialog.Builder builder = new AlertDialog.Builder(
									Sysm_conf.this);
							builder.setTitle(
						...
						if (!isgoodpass) {
							new AlertDialog.Builder(Sysm_conf.this).setMessage(
	...
	public void onClickRegisterButton(View view) {
		AlertDialog.Builder builder2 = new AlertDialog.Builder(Sysm_conf.this);
	...
	class OnClickImg implements OnClickListener {
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			SharedPreferences sharedPreferences = Sysm_conf.this
